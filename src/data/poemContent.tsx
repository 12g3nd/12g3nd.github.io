...
import { FnRef } from '../components/poetry/footnotes';
import { poems } from './poems';

/**
 * The verse itself, keyed by slug.
 *
 * Separate from poems.ts because that file has to stay importable by
 * scripts/prerenderPlugin.ts, which runs inside vite.config's module graph and
 * cannot pull React in. The same split posts.ts and src/content/*.mdx already
 * use: the record over there, the body over here.
 *
 * Markup rather than strings because the markup IS the poem — the line breaks,
 * the emphasis, and (for "A Spade of Leaves") the silhouette the lines draw.
 * The check at the bottom of this file is what stops a record without a verse
 * from shipping as a silently blank page.
 */
export const POEM_CONTENT: Record<string, ReactNode> = {
  'a-ghazal-for-gaza': (
      <>
        If you run, I'll run too. If you run to the truck, know I'll always follow. We have mere days.<br />
        So, khaalah<FnRef n={1} />, tell me, does this mean we are going to see our home's devastating fall days?<br />
        <br />
        ...
        She is sleeping. I shake her. She does not wake up. I cry; she knows I can't forever stall days.
      </>
  ),
  'strawberry': (
      <>
  ...
  'perpetual-state-of-wanting-to-sneeze': (
      <>
  ...
  'who-am-i-to-judge-another-sinner': (
      <>
        Hunger, danger<br />
  ...
  'a-spade-of-leaves-for-your-tears': (
      <>
        A<br />
        <em>sad</em><br />
...
/*
 * A poem with a record in poems.ts but no verse here renders as a blank sheet
 * of paper — which, on a page made of sheets of paper, looks deliberate and so
 * never gets reported. Checked at module load instead.
 *
 * Dev only: this is an authoring mistake, catchable the first time the page is
 * opened, and the whole branch is stripped from the production bundle.
 */
if (import.meta.env.DEV) {
  const missing = poems.filter((p) => !POEM_CONTENT[p.slug]).map((p) => p.slug);
  if (missing.length) {
    console.error(
      `[poetry] no verse for ${missing.join(', ')} — add it to POEM_CONTENT in ` +
        `src/data/poemContent.tsx, or the poem renders as an empty page.`
    );
  }
}
